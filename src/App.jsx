...
function App() {
  ...
  return (
    <main className="overflow-hidden bg-[#fffdf8] text-slate-900">
      <header className="fixed inset-x-0 top-0 z-50 border-b border-white/10 bg-rb-900/95 text-white backdrop-blur-xl">
        <div className="section-shell flex h-20 items-center justify-between gap-6">
          <a href="#home" className="flex items-center gap-3" aria-label="RB Charity Foundation home">
            <div className="grid h-11 w-11 place-items-center rounded-full bg-white text-rb-900"><Heart size={21} fill="currentColor" /></div>
            <div className="leading-tight"><strong className="block text-sm tracking-wide">RB CHARITY</strong><span className="text-xs text-white/65">FOUNDATION</span></div>
          </a>
      ...
      <section id="home" className="relative min-h-screen bg-rb-900 pt-20 text-white">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_75%_20%,rgba(217,164,65,.24),transparent_28%),radial-gradient(circle_at_10%_80%,rgba(31,122,91,.55),transparent_35%)]" />
        <div className="section-shell relative grid min-h-[calc(100vh-5rem)] items-center gap-14 py-20 lg:grid-cols-[1.15fr_.85fr]">
          <div>
      ...
      <section id="stories" className="py-24 sm:py-32">
        <div className="section-shell"><span className="eyebrow">Stories of change</span><h2 className="section-title">The people behind every campaign matter more than the numbers.</h2><div className="mt-12 grid gap-6 lg:grid-cols-3">{['Field stories and updates','Campaign milestones','Community partnerships'].map((title,i)=><article key={title} className="overflow-hidden rounded-[2rem] border border-rb-100 bg-white"><div className="aspect-[4/3] bg-gradient-to-br from-rb-100 via-rb-50 to-amber-100 p-6"><span className="text-xs font-black uppercase tracking-[.18em] text-rb-700">Story 0{i+1}</span></div><div className="p-6"><h3 className="text-xl font-black text-rb-900">{title}</h3><p className="mt-3 leading-7 text-slate-600">Real stories, photographs and verified campaign updates will appear here as the foundation publishes them.</p></div></article>)}</div></div>
      </section>

      <section id="volunteer" className="bg-[#f4ead4] py-24 sm:py-28"><div className="section-shell grid gap-10 lg:grid-cols-3"><div><HandHeart size={34} className="text-rb-700"/><h3 className="mt-5 text-3xl font-black text-rb-900">Volunteer</h3><p className="mt-3 leading-7 text-slate-700">Give your time and skills to field initiatives and community drives.</p></div><div><Users size={34} className="text-rb-700"/><h3 className="mt-5 text-3xl font-black text-rb-900">Partner</h3><p className="mt-3 leading-7 text-slate-700">Collaborate as a company, institution, NGO or community partner.</p></div><div><Heart size={34} className="text-rb-700"/><h3 className="mt-5 text-3xl font-black text-rb-900">Donate</h3><p className="mt-3 leading-7 text-slate-700">Support the general foundation fund or choose a specific verified campaign.</p></div></div></section>

      <section id="donate" className="py-24 sm:py-32"><div className="section-shell"><div className="grid overflow-hidden rounded-[2.5rem] bg-white shadow-soft lg:grid-cols-2"><div className="bg-rb-900 p-8 text-white sm:p-12"><span className="text-xs font-black uppercase tracking-[.2em] text-gold">Join the mission</span><h2 className="mt-5 text-4xl font-black tracking-[-.04em] sm:text-5xl">Your contribution can become someone else's opportunity.</h2><p className="mt-5 leading-8 text-white/70">The payment gateway will be connected only after the foundation's verified banking, legal and compliance details are available.</p></div><div className="p-8 sm:p-12"><p className="text-sm font-black uppercase tracking-[.18em] text-rb-600">Choose an amount</p><div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-4">{['₹500','₹1,000','₹2,500','Custom'].map(a=><button key={a} className="rounded-2xl border border-rb-100 px-4 py-4 font-black text-rb-900 transition hover:border-rb-600 hover:bg-rb-50">{a}</button>)}</div><button disabled className="mt-6 w-full cursor-not-allowed rounded-full bg-rb-800 px-6 py-4 font-black text-white opacity-60">Donation gateway coming soon</button><p className="mt-4 text-xs leading-5 text-slate-500">No payment will be collected until verified foundation details and gateway credentials are configured.</p></div></div></div></section>

      <footer id="contact" className="bg-[#061d17] py-14 text-white"><div className="section-shell grid gap-10 md:grid-cols-3"><div><p className="font-black">RB CHARITY FOUNDATION</p><p className="mt-3 max-w-sm text-sm leading-6 text-white/55">Business-backed social impact with a transparent pathway for public participation.</p></div><div><p className="text-sm font-black uppercase tracking-[.15em] text-white/50">Explore</p><div className="mt-4 space-y-2 text-sm text-white/70"><a className="block" href="#about">About</a><a className="block" href="#causes">Causes</a><a className="block" href="#impact">Impact</a></div></div><div><p className="text-sm font-black uppercase tracking-[.15em] text-white/50">Get involved</p><div className="mt-4 space-y-2 text-sm text-white/70"><a className="block" href="#donate">Donate</a><a className="block" href="#volunteer">Volunteer</a><span className="block">Contact details to be verified</span></div></div></div><div className="section-shell mt-10 border-t border-white/10 pt-6 text-xs text-white/40">© {new Date().getFullYear()} RB Charity Foundation. All rights reserved.</div></footer>
    </main>
  )
}
