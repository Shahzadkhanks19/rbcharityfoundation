    ...
    <main className="overflow-hidden bg-[#f8fbff] text-slate-900">
      <header className="fixed inset-x-0 top-0 z-50 border-b border-slate-200/80 bg-[#fffaf2]/95 text-rb-900 shadow-sm backdrop-blur-xl">
        <div className={`${shell} flex h-24 items-center justify-between gap-6`}>
          <Link to="/" className="flex shrink-0 items-center focus-visible:rounded-xl focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gold/70" aria-label="RB Charity Foundation home"><BrandLogo /></Link>
          <nav className="hidden items-center gap-1 xl:flex">{nav.map(([item, path]) => <Link key={path} to={path} className={navLink}><span className="relative z-10">{item}</span></Link>)}</nav>
          <div className="hidden lg:block"><Link to="/donate" className="group inline-flex items-center gap-2 rounded-full bg-gold px-5 py-3 text-sm font-black text-rb-900 shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-lg hover:shadow-orange-200/70 active:translate-y-0"><span>Donate now</span><Heart size={16} className="transition duration-300 group-hover:scale-110 group-hover:rotate-6" /></Link></div>
          <button className="rounded-xl p-2 text-rb-900 transition duration-300 hover:scale-105 hover:bg-rb-50 xl:hidden" onClick={() => setOpen(!open)} aria-label="Toggle navigation">{open ? <X /> : <Menu />}</button>
        </div>
        {open && <div className="border-t border-slate-200 bg-[#fffaf2] px-5 py-5 shadow-xl xl:hidden">{nav.map(([item, path]) => <Link onClick={() => setOpen(false)} key={path} to={path} className="block rounded-xl px-3 py-3 font-semibold text-rb-900/75 transition hover:bg-rb-50 hover:text-rb-900">{item}</Link>)}<Link onClick={() => setOpen(false)} to="/donate" className="mt-4 inline-flex w-full justify-center rounded-full bg-gold px-5 py-3 font-black text-rb-900 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">Donate now</Link></div>}
      </header>

      <section id="home" className="relative min-h-screen bg-rb-900 pt-24 text-white"><div className="absolute inset-0 bg-[radial-gradient(circle_at_75%_20%,rgba(242,162,58,.26),transparent_30%),radial-gradient(circle_at_10%_80%,rgba(55,105,166,.62),transparent_38%)]" /><div className={`${shell} relative grid min-h-[calc(100vh-6rem)] items-center gap-14 py-20 lg:grid-cols-[1.15fr_.85fr]`}><div><span className="inline-flex rounded-full border border-white/20 bg-white/10 px-4 py-2 text-xs font-bold uppercase tracking-[0.2em] text-white/80 backdrop-blur">Profits with purpose</span><h1 className="mt-6 max-w-4xl text-5xl font-black leading-[.95] tracking-[-.055em] sm:text-6xl lg:text-7xl">Turning business success into <span className="text-gold">human impact.</span></h1><p className="mt-7 max-w-2xl text-lg leading-8 text-white/70">RB Charity Foundation channels support from the wider RB group and public donors into meaningful social initiatives across food, education, healthcare and community welfare.</p><div className="mt-9 flex flex-wrap gap-4"><Link to="/donate" className="group inline-flex items-center gap-2 rounded-full bg-gold px-6 py-3.5 font-black text-rb-900 transition hover:-translate-y-1"><span>Make a difference</span><ArrowRight size={18} /></Link><Link to="/causes" className="rounded-full border border-white/20 px-6 py-3.5 font-bold text-white transition hover:-translate-y-1 hover:bg-white/10">Explore our work</Link></div></div><div className="relative mx-auto w-full max-w-lg"><div className="rounded-[2.2rem] border border-white/15 bg-white/10 p-5 shadow-2xl backdrop-blur-xl"><div className="rounded-[1.7rem] bg-white p-6 text-rb-900"><span className="text-xs font-black uppercase tracking-[.18em] text-rb-600">How RB creates impact</span><div className="mt-6 space-y-3">{['RB Group businesses grow','A share of success supports the foundation','Public donors can join the mission','Resources reach verified social initiatives'].map((item,index)=><div key={item} className="group flex items-center gap-4 rounded-2xl bg-rb-50 p-4 transition hover:translate-x-1 hover:bg-white hover:shadow-md"><span className="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-rb-800 text-sm font-black text-white">{index+1}</span><p className="font-bold">{item}</p></div>)}</div></div></div></div></div></section>
