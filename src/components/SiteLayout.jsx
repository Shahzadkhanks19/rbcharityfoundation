...
export function FloatingActions() {
  ...
  return <div className="fixed bottom-5 right-4 z-40 flex flex-col items-center gap-3 sm:bottom-7 sm:right-7">{showTop&&<button type="button" onClick={()=>window.scrollTo({top:0,behavior:'smooth'})} aria-label="Scroll to top" className="grid h-12 w-12 place-items-center rounded-full bg-rb-900 text-white shadow-xl transition duration-300 hover:-translate-y-1 hover:bg-rb-800"><ArrowUp size={20}/></button>}<Link to="/donate" aria-label="Donate now" className="group flex flex-col items-center gap-1.5 text-rb-900"><span className="grid h-14 w-14 place-items-center rounded-full bg-gold shadow-xl ring-4 ring-white/80 transition duration-300 group-hover:-translate-y-1 group-hover:scale-105 group-hover:shadow-2xl sm:h-16 sm:w-16"><Heart size={27} fill="currentColor" className="transition duration-300 group-hover:scale-110"/></span><span className="rounded-full bg-white/95 px-2.5 py-1 text-[11px] font-black shadow-md backdrop-blur sm:text-xs">Donate now</span></Link></div>
}
