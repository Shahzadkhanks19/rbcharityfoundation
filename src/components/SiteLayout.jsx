...
const nav = [['About','/about'],['Causes','/causes'],['Campaigns','/campaigns'],['Impact','/impact'],['Stories','/stories'],['Volunteer','/volunteer'],['Contact','/contact']]
...
export function SiteFooter() {
  ...
  return <footer className="border-t border-rb-100 bg-[#fffaf2] text-rb-900"><div className={`${shell} grid gap-12 py-16 lg:grid-cols-[1.1fr_.8fr_.9fr]`}><div><Link to="/"><img src={LOGO_URL} alt="RB Charity Foundation" className="h-24 w-auto max-w-[150px] object-contain object-left sm:h-28 sm:max-w-[175px]" /></Link><h2 className="mt-4 text-lg font-black">RB CHARITY FOUNDATION</h2><p className="mt-4 max-w-md leading-7 text-rb-900/60">Business-backed social impact with a transparent pathway for public participation.</p></div><div><p className="text-xs font-black uppercase tracking-[.18em] text-rb-900/55">Explore</p><div className="mt-5 flex flex-col items-start gap-1"><Link className={footerLink} to="/about">About</Link><Link className={footerLink} to="/causes">Causes</Link><Link className={footerLink} to="/campaigns">Campaigns</Link><Link className={footerLink} to="/impact">Impact</Link><Link className={footerLink} to="/stories">Stories</Link></div></div><div><p className="text-xs font-black uppercase tracking-[.18em] text-rb-900/55">Get involved</p><div className="mt-5 flex flex-col items-start gap-1"><Link className={footerLink} to="/donate">Donate</Link><Link className={footerLink} to="/volunteer">Volunteer</Link><Link className={footerLink} to="/partner">Partner</Link><Link className={footerLink} to="/contact">Contact</Link><Link className={footerLink} to="/reports">Reports</Link></div></div></div><div className="border-t border-rb-100"><div className={`${shell} flex flex-col gap-4 py-7 text-sm text-rb-900/50 sm:flex-row sm:items-center sm:justify-between`}><span>© {new Date().getFullYear()} RB Charity Foundation. All rights reserved.</span><div className="flex flex-wrap gap-4"><Link to="/privacy">Privacy</Link><Link to="/terms">Terms</Link><Link to="/refund-policy">Refund policy</Link></div></div></div></footer>
}
...
function FloatingActions() {
  ...
  return <div className="fixed bottom-5 right-4 z-40 flex flex-col items-end gap-3 sm:bottom-7 sm:right-7"><Link to="/donate" className="group inline-flex items-center gap-2 rounded-full bg-gold px-4 py-3 text-sm font-black text-rb-900 shadow-xl transition duration-300 hover:-translate-y-1 hover:shadow-2xl sm:px-5"><Heart size={18} className="transition group-hover:scale-110 group-hover:rotate-6"/><span className="hidden sm:inline">Donate now</span></Link>{showTop&&<button type="button" onClick={()=>window.scrollTo({top:0,behavior:'smooth'})} aria-label="Scroll to top" className="grid h-12 w-12 place-items-center rounded-full bg-rb-900 text-white shadow-xl transition duration-300 hover:-translate-y-1 hover:bg-rb-800"><ArrowUp size={20}/></button>}</div>
}
